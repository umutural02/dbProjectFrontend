...
const TopSideButtons = ({applySearch}) => {
    ...
    const openAddNewLeagueModal = () => {
        dispatch(openModal({title : "Add New League", bodyType : MODAL_BODY_TYPES.LEAGUE_ADD_NEW}))
    }
    ...
    return(
        <>
            ...
            <div className="inline-block float-right">
                <button className="btn px-6 btn-sm normal-case btn-primary" onClick={() => openAddNewLeagueModal()}>Add New</button>
            </div>
...
function Leagues(){
    ...
    useEffect(() => {
        ...
        axios.get(API.BASE_URL + 'getLeagueList', { headers: API.SKIP_BROWSER_WARNING })
            .then(response => {
                ...
                setLeagues(response.data.slice(indexOfFirstItem, indexOfLastItem));
    ...
    useEffect(() => {
        ...
        if (searchText) {
            updatedLeagues = updatedLeagues.filter(league =>
                league.leagueName.toLowerCase().includes(searchText.toLowerCase())
            );
        }

        // Apply sorting
        if (sortConfig.key === 'marketValue') {
            updatedLeagues.sort((a, b) => {
        ...
        setTotalItems(updatedLeagues.length);

        // Paginate the results
        const indexOfLastItem = currentPage * itemsPerPage;
        const indexOfFirstItem = indexOfLastItem - itemsPerPage;
        setLeagues(updatedLeagues.slice(indexOfFirstItem, indexOfLastItem));
    }, [sortConfig, searchText, currentPage, itemsPerPage, LEAGUES]);
    ...
    const editCurrentLeague = (index) => {
        dispatch(openModal({title : "Edit League", bodyType : MODAL_BODY_TYPES.LEAGUE_EDIT, 
        extraObject : {index, league : leagues[index]}}))
    }

    const deleteCurrentLeague = (_id) => {
        dispatch(openModal({title : "Confirmation", bodyType : MODAL_BODY_TYPES.CONFIRMATION, 
        extraObject : { message : `Are you sure you want to delete this league?`, type : CONFIRMATION_MODAL_CLOSE_TYPES.LEAGUE_DELETE, _id}}))
    }

    return(
        <>
            
            <TitleCard title="Current Leagues" topMargin="mt-2" TopSideButtons={<TopSideButtons applySearch={applySearch} />}>
            ...
            <div className="overflow-x-auto w-full">
                <table className="table w-full">
                    ...
                    <tr class="text-center">
                        <th onClick={() => handleSort('leagueName')}>Name {renderSortArrow('leagueName')}</th>
                        <th onClick={() => handleSort('teamAmount')}>Team Amount {renderSortArrow('teamAmount')}</th>
                        ...
                        <th onClick={() => handleSort('totalMarketValue')}>Total Market Value {renderSortArrow('totalMarketValue')}</th>
                    ...
                    <tbody>
                        {
                            leagues.map((l, k) => {
                                return(
                                    ...
                                    <td>{l.averageAge}</td>
                                    <td>{l.totalMarketValue}{l.totalMarketValueUnit}</td>
                                    <td><button className="btn btn-square btn-ghost" onClick={() => editCurrentLeague(k)}><PencilSquareIcon className="w-5" /></button></td>
                                    <td><button className="btn btn-square btn-ghost" onClick={() => deleteCurrentLeague(l.leagueName)}><TrashIcon className="w-5"/></button></td>
                                    </tr>
                                )
                            })
                        }
