...
const TopSideButtons = ({applySearch}) => {
    ...
    const openAddNewTeamModal = () => {
        dispatch(openModal({title : "Add New Team", bodyType : MODAL_BODY_TYPES.TEAM_ADD_NEW}))
    }
    ...
    return(
        <>
            ...
            <div className="inline-block float-right">
                <button className="btn px-6 btn-sm normal-case btn-primary" onClick={() => openAddNewTeamModal()}>Add New</button>
            </div>
...
function Teams(){
    ...
    useEffect(() => {
        ...
        axios.get(API.BASE_URL + 'getTeamList', { headers: API.SKIP_BROWSER_WARNING })
            .then(response => {
                ...
                setTeams(response.data.slice(indexOfFirstItem, indexOfLastItem));
    ...
    useEffect(() => {
        ...
        if (searchText) {
            updatedTeams = updatedTeams.filter(team =>
                team.teamName.toLowerCase().includes(searchText.toLowerCase())
            );
        }

        // Apply sorting
        if (sortConfig.key === 'totalSquadValue') {
            updatedTeams.sort((a, b) => {
                const unitIdA = a['totalSquadValueUnitId'];
                const unitIdB = b['totalSquadValueUnitId'];
                const valueA = a['totalSquadValue'];
                const valueB = b['totalSquadValue'];
        ...
        setTotalItems(updatedTeams.length);

        // Paginate the results
        const indexOfLastItem = currentPage * itemsPerPage;
        const indexOfFirstItem = indexOfLastItem - itemsPerPage;
        setTeams(updatedTeams.slice(indexOfFirstItem, indexOfLastItem));
    }, [sortConfig, searchText, currentPage, itemsPerPage, TEAMS]);
    ...
    const editCurrentTeam = (index) => {
        dispatch(openModal({title : "Edit Team", bodyType : MODAL_BODY_TYPES.TEAM_EDIT, 
        extraObject : {index, team : teams[index]}}))
    }

    const deleteCurrentTeam = (teamName) => {
        dispatch(openModal({title : "Confirmation", bodyType : MODAL_BODY_TYPES.CONFIRMATION, 
        extraObject : { message : `Are you sure you want to delete this team?`, type : CONFIRMATION_MODAL_CLOSE_TYPES.TEAM_DELETE, teamName}}))
    }

    return(
        <>
            <TitleCard title="Current Teams" topMargin="mt-2" TopSideButtons={<TopSideButtons applySearch={applySearch} />}>
            ...
            <div className="overflow-x-auto w-full">
                <table className="table w-full">
                    ...
                    <tr class="text-center">
                        <th onClick={() => handleSort('teamName')}>Name {renderSortArrow('teamName')}</th>
                        <th onClick={() => handleSort('leagueName')}>League {renderSortArrow('leagueName')}</th>
                        <th onClick={() => handleSort('totalSquadValue')}>Squad Value {renderSortArrow('totalSquadValue')}</th>
                        <th onClick={() => handleSort('playerAmount')}>Player Amount {renderSortArrow('playerAmount')}</th>
                        <th onClick={() => handleSort('foreignPlayerAmount')}>Foreign Player Amount {renderSortArrow('foreignPlayerAmount')}</th>
                        <th onClick={() => handleSort('averageAge')}>Average Age {renderSortArrow('averageAge')}</th>
                        <th onClick={() => handleSort('stadiumName')}>Stadium {renderSortArrow('stadiumName')}</th>
                    ...
                    <tbody>
                        {
                            ...
                            teams.map((l, k) => {
                                return(
                                    ...
                                    <td>{l.teamName}</td>
                                    <td>{l.leagueName}</td>
                                    <td>{l.totalSquadValue}{l.totalSquadValueUnit}</td>
                                    <td>{l.playerAmount}</td>
                                    <td>{l.foreignPlayerAmount}</td>
                                    <td>{l.averageAge}</td>
                                    <td className="overflow-hidden max-w-xs" title={l.stadiumName}>
                                        {l.stadiumName !== null && (l.stadiumName.length > 30 ? `${l.stadiumName.substring(0, 30)}...` : l.stadiumName)} 
                                    </td>
                                    <td><button className="btn btn-square btn-ghost" onClick={() => editCurrentTeam(k)}><PencilSquareIcon className="w-5" /></button></td>
                                    <td><button className="btn btn-square btn-ghost" onClick={() => deleteCurrentTeam(l.teamName)}><TrashIcon className="w-5"/></button></td>
                                    </tr>
                                )
                            })
                        }
