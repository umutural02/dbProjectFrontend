...
function AddStadiumModalBody({closeModal, extraObject}){
    ...
    useEffect(() => {
        setStadiumObj(prevStadiumObj => ({
            ...prevStadiumObj,
            oldName: extraObject.team.name
        }));
    }, [extraObject]);

    const dispatch = useDispatch()
    const [errorMessage, setErrorMessage] = useState("")
    const [stadiumObj, setStadiumObj] = useState(extraObject.team)
    ...
    const saveEditedTeam = async () => {
        ...
        if (stadiumObj.name.trim() === "")return setErrorMessage("Stadium Name is required!")
            ...
            sendEditStadiumRequest(stadiumObj)
            closeModal()
        }
    }

    const  sendEditStadiumRequest = async (newStadiumObj) => {
        try {

            const response = axios({
                method: 'post',
                url: API.BASE_URL + 'updateStadium',
    ...
    return(
        <>
            ...
            <SelectBox
                ...
                defaultValue={stadiumObj.cityId}
                updateType="cityId"
